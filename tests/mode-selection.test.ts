...
describe('composeAssembleOutro', () => {
    it('lists no extras when --with was not used', () => {
        expect(composeAssembleOutro(undefined)).toBe('Workspace assembled (tinycld). Run `pnpm install` at the root.')
    })
    it('lists no extras when --with was empty', () => {
        expect(composeAssembleOutro([])).toBe('Workspace assembled (tinycld). Run `pnpm install` at the root.')
    })
    it('appends a single --with member', () => {
        expect(composeAssembleOutro(['mail'])).toBe(
            'Workspace assembled (tinycld, mail). Run `pnpm install` at the root.'
        )
    })
    it('appends multiple --with members separated by commas', () => {
        expect(composeAssembleOutro(['mail', 'contacts'])).toBe(
            'Workspace assembled (tinycld, mail, contacts). Run `pnpm install` at the root.'
        )
    })
})
